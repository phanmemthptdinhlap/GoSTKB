...
func (h *ThaoTac_PhanCong) LayMaPhanCongTheoLop_Mon(c *gin.Context) {
	lop := c.Param("lop")
	mon := c.Param("mon")

	if lop == "" || mon == "" {
		c.JSON(http.StatusBadRequest, gin.H{"error": "Thiếu thông tin mã lớp hoặc mã môn học"})
		return
	}

	var ma_phan_cong string
	err := h.DB.QueryRow(`SELECT ma_phan_cong FROM phancong 
						WHERE ma_lop=? AND ma_mon=?`, lop, mon).Scan(&ma_phan_cong)

	if err != nil {
		if err == sql.ErrNoRows {
			c.JSON(http.StatusNotFound, gin.H{"error": "Không tìm thấy phân công cho lớp và môn học này"})
		} else {
	...
	c.JSON(http.StatusOK, gin.H{"ma_phan_cong": ma_phan_cong})
}
...
func (h *ThaoTac_PhanCong) CapNhatPhanCong(c *gin.Context) {
	...
	var exits bool
	row := h.DB.QueryRow("SELECT EXISTS(SELECT 1 FROM phancong WHERE ma_phan_cong = ?)", id)
...
func (h *ThaoTac_PhanCong) XoaPhanCong(c *gin.Context) {
	...
	// Kiểm tra lớp học tồn tại
	var exits bool
	sqler := h.DB.QueryRow("SELECT EXISTS(SELECT 1 FROM phancong WHERE ma_phan_cong = ?)", id).Scan(&exits)
	if sqler != nil {
